...
const ProductList = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:4567/api/products/fetch-product')
  ...
  const scroll = (category, dir) => {
    scrollRefs.current[category]?.scrollBy({ left: dir * 300, behavior: 'smooth' })
  }
    ...
    <div className='px-6 sm:px-16 lg:px-24 py-12'>
      ...
      {Object.entries(grouped).map(([cat, items]) => (
        <div key={cat} className='mb-8'>
          <h3 className='text-black text-base sm:text-lg font-medium mt-5 mb-4'>{cat}</h3>
          <div className='relative'>
            {showLeft[cat] && (
              <button onClick={() => scroll(cat, -1)} className='absolute left-0 top-1/2 -translate-y-1/2 p-1 z-10'>
            ...
            >
              {items.map(p => (
                ...
                >
                  ...
                  <p className='text-black text-xs sm:text-sm font-medium mt-3 text-left'>{p.name}</p>
                  <p className='text-gray-500 text-xs sm:text-sm text-left'>₹{p.price}</p>
                </div>
              ))}
            </div>

            <button onClick={() => scroll(cat, 1)} className='absolute right-0 top-1/2 -translate-y-1/2 p-1 z-10'>
