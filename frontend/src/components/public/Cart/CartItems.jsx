...
const CartItems = () => {
  const { cart, removeFromCart, addToCart } = useCart(); // Use Cart Context
  const navigate = useNavigate()

  const changeQuantity = (id, amount) => {
    const item = cart.find((product) => product.id === id);
    if (item) {
      addToCart(item, amount); // Increase or decrease quantity using CartContext
    }
  }
        ...
        {cart.length === 0 ? (
          ...
          <>
                ...
                <tbody>
                  {cart.map(({ id, image, name, price, quantity }) => (
                    <tr key={id} className='border-b border-gray-200'>
                      ...
                      <td className='p-3'>
                        <button onClick={() => changeQuantity(id, -1)} className='px-2 border border-gray-300 rounded'>
                          -
                        </button>
                      ...
                      <td className='p-3'>
                        <button onClick={() => removeFromCart(id)} className='text-red-600'>Remove</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className='text-right mt-4'>
              <button onClick={() => navigate(`/checkout/${total}`)} className='mt-6 px-4 py-2 border border-green-600 text-green-600 hover:bg-green-600 hover:text-white rounded transition'>
                Proceed to Checkout
              </button>
            </div>
          </>
